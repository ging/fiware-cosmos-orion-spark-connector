 ...
 */
class OrionHttpHandlerLD(callback: NgsiEventLD => Unit) extends ChannelInboundHandlerAdapter {
  ...
  def parseMessage(req : FullHttpRequest) : NgsiEventLD =  {
    try {
      // Retrieve headers
      ...
      // Generate NgsiEvent
      val ngsiEvent = NgsiEventLD(creationTime, service, servicePath, entities)
      ngsiEvent
    } catch {
