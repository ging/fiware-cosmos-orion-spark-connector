  ...
  class OrionSubscription(entity: String, cbUrl: String, description: String, callbackHost: String, callbackPort: Int, content: String = ""  ) {
    implicit val formats = DefaultFormats
    private lazy val logger = LoggerFactory.getLogger(getClass)
    var subscriptionId = ""
    def this(entity: String, cbUrl: String, description: String, callbackHost: String, callbackPort: Int) {
      this(entity,
        OrionSubscriptionHelpers.getOrionHost(cbUrl),
        description,
        OrionSubscriptionHelpers.getCallbackHost(callbackHost, true),
        callbackPort,
        OrionSubscriptionHelpers.createContent(entity, description, callbackHost, callbackPort))
    }
  ...
  object OrionSubscriptionHelpers {
    ...
    def getCallbackHost(callbackHost : String, log: Boolean = false): String = {
      if (callbackHost.isEmpty) {
        ...
        if (NOTIFICATION_HOST == host) {
          if (log) logger.info("Callback host not specified nor found in env variable NOTIFICATION_HOST. Using " + NOTIFICATION_HOST)
        } else {
    ...
    def getOrionHost(orionHost : String, log: Boolean = false): String = {
      if (orionHost.isEmpty) {
        val NOTIFICATION_HOST = scala.util.Properties.envOrElse("ORION_HOST", "localhost")
        logger.info("Orion: " + NOTIFICATION_HOST)
        return NOTIFICATION_HOST
      }
    ...
    def createContent(entity: String,
                      description: String,
                      callbackHost: String,
